...
def _mem_search(inp: MemSearchIn) -> str:
    ...
    if not m.is_available():
        return format_tool_result({"results": [], "disabled": True, "error": m.get_unavailable_reason()}, [])
...
def _mem_get(inp: MemGetIn) -> str:
    m = MemoryManager.get()
    d = m.get(path=inp.path, from_=inp.from_, lines=inp.lines)
...
def _mem_upd(inp: MemUpdIn) -> str:
    ...
    if inp.action == "edit":
        if not inp.old_text or inp.new_text is None:
            return format_tool_result({"success": False, "error": "old_text and new_text required"})
...
def memory_search_tool() -> StructuredTool:
    return StructuredTool.from_function(
        name="memory_search", description=MEMORY_SEARCH_DESCRIPTION, func=_mem_search, args_schema=MemSearchIn
    )
