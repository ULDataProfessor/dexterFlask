...
def test_agent_run_no_tools_yields_done() -> None:
    ...
    agent = Agent(config=AgentConfig(model="gpt-5.4"), tools=[], system_prompt="SYS")
...
def test_agent_run_tool_path_emits_thinking_and_done(monkeypatch) -> None:
    ...
    def fake_execute_all(self: Any, response: Any, ctx: Any) -> Generator[dict[str, Any], None, None]:
